...
class Media {
  ...
  update(scroll: any, direction: string) {
    ...
    const scaleBoost = lerp(1.4, 0.6, Math.min(distFromCenter, 1));
    this.plane.scale.y = ((this.viewport.height * (750 * this.scale)) / this.screen.height) * scaleBoost;
    this.plane.scale.x = ((this.viewport.width * (600 * this.scale)) / this.screen.width) * scaleBoost;
...
class GalleryApp {
  ...
  constructor(container: HTMLElement, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, autoScrollSpeed = 0.5 }: any = {}) {
